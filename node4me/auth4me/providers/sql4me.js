...
module.exports = class sql4me {	
	...
	init(){
		...
		this.database.query("USE" + this.field.database + ";");
	...
	signin(username, password){
		let query = this.select("SELECT " + this.field.id + ", " + this.field.password + " FROM " + this.field.table + " WHERE " + this.field.username + " = ? OR " + this.field.email + " = ?;", "ss", username, username);
		return query[0] ? bcrypt.compareSync(password, query[1][1]) ? [true, query[1][0]] : [false, "Incorrect username or password!"] : [bcrypt.hashSync(password, this.bcrypt) ? false : false, "Incorrect username or password!"];
	}
		
	signup(username, password, email){
		// TODO: Make the function duration the same for both entries
		if(!this.select("SELECT " + this.field.id + " FROM " + this.field.table + " WHERE " + this.field.username + " = ? OR " + this.field.email + " = ?;", [username, username])[0]){
			return [this.insert("INSERT INTO " + this.field.table + " (" + this.field.username + ", " + this.field.email + ", " + this.field.password + ") VALUES (?, ?, ?);", [username, email, bcrypt.hashSync(password, this.bcrypt)])[0], "Account created successfully!"];
		}
		return [false, "There was an error trying to create your account."];
	}
		
	password(id, password){
		return this.insert("UPDATE `" + this.field.table + "` SET `" + this.field.password + "` = ? WHERE `" + this.field.id + "` = ?;", [bcrypt.hashSync(password, this.bcrypt), id])[0] ? [true, "Password was set sucessfully!"] : [false, "There was an error trying to set the password."];
	}
		
	email(id, email){
		return this.insert("UPDATE `" + this.field.table + "` SET `" + this.field.email + "` = ? WHERE `" + this.field.id + "` = ?;", [email, id])[0] ? [true, "E-mail was set sucessfully!"] : [false, "There was an error trying to set the e-mail."];
	}
	
	select(query, bind){
		var empty = false, fields;
		this.database.query(query, bind, function (error, results) {
			!error && results.length ? fields = results : empty = true;
		})	
